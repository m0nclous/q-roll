...
trait YFYM_T_Variable_Get_Description {
	...
	public function get_description($tag_name = 'description', $result_xml = '') {
		...
		if (!empty($description_yml)) {
			...
			$description_yml = strip_shortcodes($description_yml);
			$description_yml = apply_filters('yfym_description_filter', $description_yml, $product->get_id(), $product, $this->get_feed_id());
			$description_yml = apply_filters('yfym_description_filter_variable', $description_yml, $product->get_id(), $product, $offer, $this->get_feed_id()); /* с версии 3.2.6 */
			$description_yml = trim($description_yml);

			$description_yml = apply_filters('y4ym_f_variable_val_description', $description_yml, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
			if ($description_yml !== '') {
				$result_xml .= '<description><![CDATA['.$description_yml.']]></description>'.PHP_EOL;
			}
			...
			if (!empty($result_yml_desc)) {$result_xml .= $result_yml_desc;}
		}

		$result_xml = apply_filters('y4ym_f_variable_tag_description', $result_xml, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
		return $result_xml;
	}
