...
if (!in_array($plugin, apply_filters('active_plugins', get_option('active_plugins', array()))) && !(is_multisite() && array_key_exists($plugin, get_site_option('active_sitewide_plugins', array())))) {
	add_action('admin_notices', 'yfym_warning_notice');
	return;
}
/**
...
* @since 3.7.0
*/
function yfym_warning_notice() {
	$class = 'notice notice-error';
	$message = 'Yml for Yandex Market '. __('requires WooCommerce installed and activated', 'yfym');
...
/* ! */	add_action('woocommerce_process_product_meta',  array($this, 'yfym_art_woo_custom_fields_save'), 10, 1);
	...
	public function save_post_product_func($post_id, $post, $update) {
		...
		for ($i = 0; $i < count($yfym_settings_arr_keys_arr); $i++) {
			...
			if ($yfym_ufup !== 'on') {$feed_id++; continue; /*return;*/}
			...
			if ($yfym_date_save_set > $last_upd_file) {
				...
				$yfym_status_cron = yfym_optionGET('yfym_status_cron', $feed_id);
				...
				new YFYM_Error_Log('FEED № '.$feed_id.'; yfym_cron_period внесен в список заданий! Файл: yml-for-yandex-market.php; Строка: '.__LINE__);
				...
				yfym_clear_file_ids_in_yml($feed_id); /* С версии 2.0.0 */
		/* ? */
	...
	public function yfym_do_this_event($feed_id = '1') {
